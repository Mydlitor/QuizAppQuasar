...
const onMoveCategoryUp = (index) => {
    if (index <= 0) return;
    [questionData.value.categories[index - 1], questionData.value.categories[index]] = [
        questionData.value.categories[index],
        questionData.value.categories[index - 1],
    ];
};

const onMoveCategoryDown = (index) => {
    if (index >= questionData.value.categories.length - 1) return;
    [questionData.value.categories[index + 1], questionData.value.categories[index]] = [
        questionData.value.categories[index],
        questionData.value.categories[index + 1],
    ];
...
const onRemoveQuestion = () => {
    ...
    questionData.value.categories[selectedContextCategoryIndex.value].questions.splice(
        selectedContextQuestionIndex.value,
        1,
    );
};
